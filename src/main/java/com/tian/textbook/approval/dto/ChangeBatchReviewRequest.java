 ...
 * reject 时 reason 必填 1-200 字。@Size 与 DDL 列宽一致
 * （change_request.batch_no VARCHAR(40) / reason VARCHAR(200)）。</p>
 */
public record ChangeBatchReviewRequest(
        /** 批次号（与 change_request.batch_no / import_batch.batch_no 对应） */
        @NotBlank(message = "批次号不能为空") @Size(max = 40, message = "批次号不能超过 40 字符") String batchNo,
        /** 审批动作：pass/reject */
        @NotBlank(message = "审批动作不能为空") @Size(max = 16, message = "审批动作不合法") String action,
